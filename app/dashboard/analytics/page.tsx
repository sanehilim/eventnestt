...
export default function AnalyticsPage() {
  ...
  const { payments, totalRevenue } = useOrganizerRevenue()
  ...
  const analyticsData = useMemo(() => {
    ...
    const recentPayments = payments.slice(-4).reverse().map((payment) => {
      ...
      return {
        type: "ticket_sale",
        event: event?.name || `Event #${payment.eventId}`,
        detail: "Paid ticket minted",
        amount: formatPayment(payment.amountWei),
      }
    })
    const recentActivity = recentPayments.length > 0
      ? recentPayments
      : events
          .slice()
          ...
          .map((event) => {
            ...
            return {
              ...
              event: event.name,
              detail: isGated ? "Access rules active" : "Public registration available",
              amount: null,
            }
          })
      ...
      <div className="pt-28 pb-20">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-12">
            ...
            {!isConnected && <WalletConnectButton label="Connect Wallet" compact />}
          </div>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                ...
                <div className="bg-[#f5f5f5] rounded-lg p-6 border border-[#e5e5e5]">
                  <div className="flex items-center justify-between mb-4">
                      ...
                      {Math.round((analyticsData.gatedEvents / analyticsData.totalEvents) * 100)}%
                    </span>
                  </div>
                  <p className="text-[#666666] text-sm mb-1">Gated Events</p>
                  <p className="text-3xl font-semibold text-[#1a1a1a]">{analyticsData.gatedEvents}/{analyticsData.totalEvents}</p>
                </div>
              </div>
                  ...
                  <div className="flex items-end justify-between h-40 gap-2">
                    {analyticsData.ticketSales.map((item, index) => (
                      <div key={item.fullName} className="flex-1 flex flex-col items-center gap-2">
                        <div
                          className="w-full bg-[#0f766e] rounded-t-lg transition-all duration-500"
                    ...
                    {analyticsData.recentActivity.map((activity, index) => (
                      <div key={`${activity.event}-${index}`} className="flex items-center justify-between p-3 bg-white rounded-xl border border-[#e5e5e5]">
                        <div className="flex items-center gap-3">
                          ...
                          {activity.type === "new_event" && <Calendar className="w-4 h-4 text-[#0f766e]" />}
                          {activity.type === "access_granted" && <Lock className="w-4 h-4 text-[#0f766e]" />}
                          <div>
